...
function _renderSobreSidebar(container) {
  ...
  container.innerHTML = `
    <style>
      ...
      .sc-section-head { padding:0.6rem 0.75rem; font-size:0.7rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.08em; display:flex; align-items:center; justify-content:center; text-align:center; }
      .sc-row { padding:0.4rem 0.75rem; display:flex; flex-direction:column; gap:0.2rem; }
      .sc-label { font-size:0.65rem; color:var(--text-muted); font-weight:600; text-transform:uppercase; letter-spacing:0.05em; text-align:center; display:block; width:100%; }
      ...
      .sc-range-val { font-size:0.65rem; font-family:monospace; color:var(--text-muted); min-width:2.2rem; text-align:right; }
  ...
  const _renderLayerList = () => {
    ...
    if (!layers.length) {
      list.innerHTML = '<p style="padding:0.75rem; color:var(--text-secondary); font-size:0.7rem; text-align:center;">Nenhuma foto adicionada</p>';
      return;
    }
    list.innerHTML = [...layers].reverse().map((l, idx) => `
      <div class="sc-layer-item ${l.id === _sobreSelectedLayerId ? 'active' : ''}" data-id="${l.id}" data-idx="${idx}" draggable="true">
        <span style="cursor:grab; color:var(--text-secondary); font-size:1rem; flex-shrink:0;" class="layer-drag">⠿</span>
