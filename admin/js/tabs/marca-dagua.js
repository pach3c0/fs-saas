...
let _canvasEls = []; // Array of canvas elements
let _panelEl = null;
let _saveTimeout = null;
let _containerRef = null;
let _platformWatermarkBg = '';
...
function renderCanvas() {
  if (!_canvasEls || _canvasEls.length === 0) return;
  
  _canvasEls.forEach(canvasEl => {
    canvasEl.querySelectorAll('.wm-layer').forEach(el => el.remove());
...
function startDrag(e, id, canvasEl) {
  e.preventDefault();
...
function startResize(e, id, dir) {
  e.preventDefault();
  e.stopPropagation();
  const canvasEl = e.target.closest('.wm-canvas-container');
  const canvas = canvasEl ? canvasEl.getBoundingClientRect() : { width: 400, height: 300 };
  const layer = wmLayers.find(l => l.id === id);
  if (!layer) return;
    ...
    _panelEl.innerHTML = `
      <div style="display:flex;flex-direction:column;gap:0.75rem;padding:0.75rem;">
          ...
          <div>
            ...
            <div style="display:flex;gap:0.25rem;">
              ${['light','normal','bold'].map(w => `
                <label style="flex:1;text-align:center;padding:0.3rem 0;border:1px solid var(--border);border-radius:0.25rem;cursor:pointer;font-size:0.6875rem;font-weight:${w==='bold'?'700':w==='light'?'300':'400'};transition:all 0.1s;${fw===w?'background:var(--accent);color:var(--accent-on);border-color:var(--accent);':'color:var(--text-primary);'}">
        ...
        <div style="display:flex;gap:0.5rem;">
          <label style="display:flex;align-items:center;gap:0.25rem;cursor:pointer;font-size:0.75rem;padding:0.25rem 0.625rem;border:1px solid var(--border);border-radius:0.25rem;${layer.fontStyle==='italic'?'background:var(--accent);color:var(--accent-on);border-color:var(--accent);':'color:var(--text-primary);'}">
            <input type="checkbox" id="lp-italic" ${layer.fontStyle==='italic'?'checked':''} style="display:none;"><em>Itálico</em>
          </label>
          <label style="display:flex;align-items:center;gap:0.25rem;cursor:pointer;font-size:0.75rem;padding:0.25rem 0.625rem;border:1px solid var(--border);border-radius:0.25rem;${layer.shadow?'background:var(--accent);color:var(--accent-on);border-color:var(--accent);':'color:var(--text-primary);'}">
        ...
        <div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.25rem;">
            ${[['none','Original','🎨'],['grayscale','P&B','⬛'],['invert','Invertido','🔄'],['white','Branco','⬜']].map(([v,l,icon]) => `
              <label style="text-align:center;padding:0.375rem;border:1px solid var(--border);border-radius:0.25rem;cursor:pointer;font-size:0.6875rem;transition:all 0.1s;${fi===v?'background:var(--accent);color:var(--accent-on);border-color:var(--accent);':'color:var(--text-primary);'}">
...
function _bindPanelEvents(layer) {
  ...
  const toggleLabel = (el, active) => {
    ...
    el.style.color = active ? 'var(--accent-on)' : 'var(--text-primary)';
...
function renderLayerList(container) {
  ...
  [...wmLayers].reverse().forEach((layer, revIdx) => {
    ...
    const label = layer.type === 'text'
      ? `✏️ ${escapeHtml((layer.text || 'Texto').slice(0, 26))}`
      : `🖼️ Imagem`;
    const el = document.createElement('div');
...
export async function renderMarcaDagua(container) {
  _containerRef = container;
  try {
    const [profileRes, configRes] = await Promise.all([
      apiGet('/api/organization/profile'),
      apiGet('/api/platform/config').catch(() => ({ watermarkPreviewImage: '' }))
    ]);
    organizationData = profileRes.data || profileRes;
    _platformWatermarkBg = configRes.watermarkPreviewImage ? resolveImagePath(configRes.watermarkPreviewImage) : '';
  } catch (error) {
    container.innerHTML = `<p style="color:var(--red);">Erro ao carregar dados.</p>`;
    return;
  }
  ...
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:2.5rem;animation:fadeInUp 0.3s ease;">
      <h2 style="font-size:1.5rem;font-weight:bold;color:var(--text-primary);">Configurar Marca D'água</h2>
      ...
      <div style="background:var(--bg-surface);padding:1.5rem;border-radius:0.5rem;border:1px solid var(--border);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:0.5rem;">
          <div>
            <h3 style="font-size:1.25rem;font-weight:600;color:var(--text-primary);margin:0 0 0.2rem;">Camadas</h3>
          ...
          <div style="display:flex;gap:0.375rem;flex-wrap:wrap;align-items:center;">
            <button id="wmAddText" style="background:var(--accent);color:var(--accent-on);border:none;padding:0.375rem 0.875rem;border-radius:0.375rem;font-size:0.8125rem;font-weight:600;cursor:pointer;">+ Texto</button>
        ...
        <div style="display:grid;grid-template-columns:1fr 260px;gap:1.25rem;align-items:start;">
          ...
          <div>
            <div style="display:flex;gap:0.375rem;margin-bottom:0.5rem;align-items:center;">
              <span style="font-size:0.6875rem;color:var(--text-secondary);">Fundo de Teste:</span>
              ...
              <button class="wm-bg-btn" data-bg="image" style="width:20px;height:20px;border-radius:3px;border:2px solid transparent;cursor:pointer;background:url('${_platformWatermarkBg}') center/cover;" title="Imagem Real"></button>
            </div>
            <div style="display:flex; gap:1.5rem; align-items:center; justify-content:center; background:var(--bg-base); padding:1.5rem; border-radius:0.5rem; border:1px dashed var(--border); flex-wrap:wrap;">
              <div style="width: 280px; max-width: 100%;">
                <div style="font-size:0.6875rem;color:var(--text-secondary);text-align:center;margin-bottom:0.375rem;">Paisagem (16:9)</div>
                <div class="wm-canvas-container" style="position:relative;width:100%;aspect-ratio:16/9;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);border-radius:0.5rem;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.3);"></div>
              </div>
              <div style="width: 160px; max-width: 100%;">
                <div style="font-size:0.6875rem;color:var(--text-secondary);text-align:center;margin-bottom:0.375rem;">Retrato (9:16)</div>
                <div class="wm-canvas-container" style="position:relative;width:100%;aspect-ratio:9/16;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);border-radius:0.5rem;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.3);"></div>
              </div>
            </div>
  ...
  _canvasEls = Array.from(container.querySelectorAll('.wm-canvas-container'));
  ...
  // set default bg to image if available
  if (_platformWatermarkBg) {
    const bgImage = `url('${_platformWatermarkBg}')`;
    _canvasEls.forEach(c => {
      c.style.backgroundImage = bgImage;
      c.style.backgroundSize = 'cover';
      c.style.backgroundPosition = 'center';
    });
    container.querySelectorAll('.wm-bg-btn').forEach(b => {
      b.style.borderColor = b.dataset.bg === 'image' ? 'var(--accent)' : 'transparent';
    });
  }
  ...
  container.querySelectorAll('.wm-bg-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const bgs = {
        ...
        image: `url('${_platformWatermarkBg}')`
      };
      _canvasEls.forEach(c => {
        c.style.background = bgs[btn.dataset.bg] || bgs.dark;
